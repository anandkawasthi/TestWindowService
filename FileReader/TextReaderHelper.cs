    ...
    public static class TextReaderHelper
    {
        ...
        private static void ParseLineData(Eob obj, List<KeyValuePair<int, List<string>>> keyValuePairs)
        {
            ...
            keyValuePairs.ForEach(x =>
            {
                ...
                var tempAllowedAmt = Convert.ToDecimal(objLineItem.ChargeAmt) - Cobligatition;
                objLineItem.AllowedAmount = tempAllowedAmt.ToString();
                var PatientRsponsibility = tempAllowedAmt - Convert.ToDecimal(objLineItem.PaymentAmt);
                objLineItem.PatientRsponsibility = PatientRsponsibility.ToString();
